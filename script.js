...
if (jobTitleElement) {
    ...
    if (hamburger && navLinks) {
        const searchListItem = document.querySelector('.search-trigger-btn')?.parentElement;
        ...
        hamburger.addEventListener('click', () => {
            ...
            // إخفاء زر البحث عند فتح القائمة في الموبايل وإظهاره عند الإغلاق
            if (searchListItem && window.innerWidth <= 992) {
                searchListItem.style.display = navLinks.classList.contains('active') ? 'none' : 'list-item';
            }
        });
    ...
    // --- ب) نظام المستخدمين (Firebase Auth) ---
    ...
    if(loginBtn) {
        loginBtn.addEventListener('click', (e) => {
            e.preventDefault();
            if (!auth) {
                alert("خدمة التسجيل غير متاحة حالياً.");
                return;
            }
    ...
    window.checkAndDeductCredit = async function() {
        ...
        if (userCredits > 0) {
            userCredits--;
            updateCreditsUI();
            if(db) await db.collection('users').doc(currentUser.uid).update({ credits: userCredits });
            return true;
        } else {
            alert("لقد نفذ رصيدك المجاني. يرجى الترقية.");
            return false;
        }
    };
